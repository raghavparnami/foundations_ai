...
export default function HistoryList() {
  ...
  function newChat() {
    const fresh = "c-" + Math.random().toString(36).slice(2, 10) + Date.now().toString(36).slice(-4);
    router.push(`/?c=${fresh}`);
  }

  function open(c: Convo) {
    // Always route to / with the conversation id, even if we're on /admin etc.
    if (pathname === "/") {
      ...
      {convos.length === 0 ? (
        ...
        groups.map(({ label, items }) => (
          <div key={label} className="mb-2">
            ...
            <ul>
              {items.map((c) => {
                const active = c.id === currentSlug && pathname === "/";
                return (
                  <li key={c.id}>
                    <button
                      title={c.title}
